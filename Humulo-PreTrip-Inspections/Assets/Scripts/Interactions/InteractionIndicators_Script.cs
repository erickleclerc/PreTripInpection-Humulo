...
public class InteractionIndicators_Script : MonoBehaviour
{
    public Camera cam;
    ...
    public GameObject sightIndicator, grabIndicator, loadedBar;
    ...
    void Update()
    {
        ...
        if(Physics.SphereCast(cam.transform.position, cameraCastRadius, cam.transform.forward, out hit, cameraCastDistance, includedLayers))
        {
            ...
            {
                Debug.Log("Grabbable on sight " + hit.collider.gameObject.name);
                ...
                else if(interactableObj.interactionType == "sight" && interactableObj.shouldBeInteractedWith)
                {
                    ...
                    sightIndicator.transform.position = worldPoint;
                    interactableObj.OnSight();

                    loadedBar.transform.localScale = new Vector3(interactableObj.timeOnSight/interactableObj.timeToCompleteSight, loadedBar.transform.localScale.y, loadedBar.transform.localScale.z); //scale loaded bar depending of time on sight
                }
                else
                {
